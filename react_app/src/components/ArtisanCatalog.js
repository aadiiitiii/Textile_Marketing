...
import ArtisanCard from './layout/ArtisanCard';
...
class ArtisanCatalog extends Component {
  ...
  componentDidMount() {
    ...
    console.log(user);
    if (user.data && user.data.realm === 'designer') {
      axios
        .get(
          `http://localhost:8080/api/profiles?filter=%7B%22where%22%3A%7B%22realm%22%3A%22artisan%22%7D%7D`,
          {}
        )
        .then(res => {
          // on success
  ...
  render() {
    ...
    return (
      <div style={{ marginTop: '10vw' }}>
        <Grid container>
          {jsondata.map(value => (
            <ArtisanCard carddata={value} user={user} alldata={jsondata} />
          ))}
        </Grid>
      </div>
    );
